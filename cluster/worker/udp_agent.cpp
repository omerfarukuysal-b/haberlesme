...
void UdpAgent::run(uint8_t myId, const sockaddr_in &masterAddr, CommandHandler &handler, TelemetryCollector &collector)
{
    std::atomic<uint32_t> seq{1};
    ...
    using namespace std::chrono_literals;
    while (g_running) {
      // collector artık parametre olarak yukarıdan geliyor, burada direkt kullanıyoruz
      std::string p = heartbeat::make_payload(collector); 
      auto bytes = proto::encode(proto::MsgType::Heartbeat, myId, seq++, 0, p);
